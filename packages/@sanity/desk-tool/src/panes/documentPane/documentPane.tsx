...
import {createObservableController, Timeline} from './history'
...
export const DocumentPane = withInitialValue((props: Props) => {
  ...
  const historyController = useObservable(
    createObservableController({timeline, documentId, client})
  ...
  if (startTime) {
    // timeline.setRange(startTime, null)
    displayedValue =
      historyDisplayed === 'from' ? timeline.startAttributes() : timeline.endAttributes()
  }
